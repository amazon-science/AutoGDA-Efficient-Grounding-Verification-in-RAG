...
from src.utils.script_utils import get_datasets, get_query_model_for_dataset, init_population_from_df, init_population_from_dump, get_validation_evidence_size
...
def run_from_config(config, dataset: str, group: str, run: str, save_path="runs"):
    ...
    querymode = get_query_model_for_dataset(dataset, group) # query integration model, i.e., how to use the queries.

    ## Perform evidence selection and integrate the queries in the evidence
    data_train.df = perform_query_integration(data_train.df, mode=querymode)
    list_evidence = data_train.df.evidence.unique()[:config["evidences"]]
    data_train.df = data_train.df[data_train.df['evidence'].isin(list_evidence)]

    data_test.df = perform_query_integration(data_test.df, mode=querymode)
    data_val.df = perform_query_integration(data_val.df, mode=querymode)
    ...
    for mutation_args in config["mutations"]:
        if "active" not in mutation_args or mutation_args["active"] is True:
            if test_ft_target:
                ...
                mutation_obj = get_mutation(mutation_args, persistent_model)
            if config["mutation_cache"] is False:
                mutation_list.append(mutation_obj)
            else: # wrap in cache
                mutation_list.append(MutationCache(mutation_obj, dataset_name=f"{dataset}-{group}"))
