...
class OffspringSelector(ABC):
    """ Interface for the offspring selection strategy.
...
class IndependentSelector(OffspringSelector):
...
class RandomSelector(OffspringSelector):
    """ Solve an independent version of the optimization problem. """
    def __init__(self, target_population: PopulationWithEmbeddings, utility_fn=None):
        self.target_population = target_population
        self.utility_fn = utility_fn
    ...
    def select(self, mypopulation_embedds: PopulationWithEmbeddings, num_select: int) -> tp.List[str]:
        ...
        ret_dict = {}
        for tag in mypopulation_embedds.tags:
            index_dict = np.random.permutation(len(mypopulation_embedds[tag]))
            ret_dict[tag] = index_dict
        return ret_dict
