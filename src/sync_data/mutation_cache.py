...
def repair_cache(cache):
    for clm in cache.columns:
        if clm == "tag_1":
            typing = cache.tag_1.apply(is_int_try)
        elif clm in ["old_claim", "new_claim", "tag_0"]:
            typing = cache[clm].apply(is_str)
        elif clm == "used":
            typing = cache.used.apply(is_bool_try)
        elif clm.startswith("rmiss"):
            typing = cache[clm].apply(is_float_try)
        else:
            raise ValueError("Unexpected clm.")
        cache = cache[typing==True]
        #print(len(cache), clm)
    ## Now actually cast types
    for clm in cache.columns:
        if clm == "tag_1":
            cache.tag_1 = cache.tag_1.astype(int)
        elif clm in ["old_claim", "new_claim", "tag_0"]:
            cache[clm] = cache[clm].astype(str)
        elif clm == "used":
            cache[clm] = cache[clm].astype(bool)
        elif clm.startswith("rmiss"):
            cache[clm] = cache[clm].astype(float)
        else:
            raise ValueError("Unexpected clm.")
    return cache
...
class MutationCache:
    def __init__(self, base_mutation: ProbCorrectMutation, dataset_name: str):
        ...
        if os.path.exists(self.cache_path):
            self.cache = pd.read_csv(self.cache_path, on_bad_lines="skip", encoding_errors="ignore")
            self.cache = repair_cache(self.cache)
    ...
    def update_cache(self, population_org, population_update, rmiss_dict):
        ...
        if os.path.exists(self.cache_path):
            prior_cache = pd.read_csv(self.cache_path, on_bad_lines="skip", encoding_errors="ignore")
            prior_cache = repair_cache(prior_cache)
