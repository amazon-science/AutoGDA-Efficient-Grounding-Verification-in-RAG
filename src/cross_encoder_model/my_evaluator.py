        ...
        if isinstance(model, CrossEncoder):
            ...
            with torch.no_grad():
                if type(model).__name__ == "HHEMv2ForSequenceClassification":
                    ...
                    x = self.tokenizer([model.prompt.format(**pair) for pair in pair_dict], return_tensors='pt', padding=True)
                else:
                    x = self.tokenizer(self.sentence_pairs, return_tensors='pt', padding=True, truncation=False)
                batch_sz = 4
                scores_list= []
