...
class DataCollatorWithTokenization:
    ...
    def __call__(self, batch):
        # print(batch)
        x = self.tok(list([[b["evidence"], b["claim"]] for b in batch]), return_tensors='pt', padding=True, truncation=True)
...
class Vectara2DataCollatorWithTokenization:
    ...
    def __call__(self, batch):
        ...
        x = self.tok([self.prompt.format(**pair) for pair in pair_dict], return_tensors='pt', padding=True, truncation=True)
