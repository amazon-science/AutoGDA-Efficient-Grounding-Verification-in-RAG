...
METRIC="roc"
def perform_run_with_params(train_dict):
    if not train_dict["use_llmpartialrephrase"] and not train_dict["use_localrephrase"] \
            and not train_dict["use_sentencedel"] and not train_dict["use_llmintro"]:
        return {METRIC: 0.5}
    ...
    base_config["mutations"][0]["active"] = train_dict["use_llmpartialrephrase"]
    base_config["mutations"][0]["entail_model"] = train_dict["rmiss_model"]
    base_config["mutations"][1]["active"] = train_dict["use_localrephrase"]
    base_config["mutations"][1]["entail_model"] = train_dict["rmiss_model"]
    base_config["mutations"][2]["active"] = train_dict["use_sentencedel"]
    base_config["mutations"][2]["entail_model"] = train_dict["rmiss_model"]
    base_config["mutations"][3]["active"] = train_dict["use_llmintro"]
    base_config["mutations"][3]["entail_model"] = train_dict["pinit_model"]
    ...
    log_path = os.path.join(f"hyperopt-{args.dataset}-{args.group}", runid)
    os.makedirs(log_path, exist_ok=True)
    json.dump(train_dict, open(f"{log_path}/hyperconfig.json", "w"))
    try:
        res = run_from_config(base_config, args.dataset, args.group, runid, save_path=f"hyperopt-{args.dataset}-{args.group}")
...
def perform_run(trial):
    ## Get values
    hyperparameters = json.load(open("src/scripts/hyperparameters_v2.json"))
    train_dict = {}
    ...
    run_result = perform_run_with_params(train_dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--n_trials", type=int, default=80, help="how many trials.")
