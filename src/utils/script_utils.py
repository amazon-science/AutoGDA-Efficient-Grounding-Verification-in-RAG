...
def get_datasets(base_dataset = "ragtruth", group="Summary", filter_model_str="facebook/bart-large-mnli", length_limit=1022):
    ...
    if base_dataset == "ragtruth":
        ...
        if group == "all":
            ...
            group_use = group
        data_test = get_summedit_group_dataset(subset="test", group=group_use, filter_length=True,
                                          filter_model_str=filter_model_str, length_limit=length_limit)
        data_train = get_summedit_group_dataset(subset="train", group=group_use, filter_length=True,
                                          filter_model_str=filter_model_str, length_limit=length_limit)
        ...
        data_test = get_lfqa_verification(split="test", group="all", filter_length=True,
                                      filter_model_str=filter_model_str, length_limit=length_limit)
        data_train = get_lfqa_verification(split="train", group="all", filter_length=True,
                                       filter_model_str=filter_model_str, length_limit=length_limit)
        data_val = get_lfqa_verification(split="val", group="all", filter_length=True,
                                     filter_model_str=filter_model_str, length_limit=length_limit)
    else:
        raise ValueError(f"Unknown dataset {base_dataset}")
    return data_train, data_test, data_val

def init_population_from_df(df, labeled=True, use_evidence=None, max_per_evidence=None,
                            seed=1, quantile_scores=False):
    ...
    if quantile_scores and "p_init" in df.columns:
        df["p_init"] = df["p_init"].values.argsort().argsort()/len(df)
...
def init_population_from_dump(df, labeled=True, use_evidence=None, max_per_evidence=None,
                              seed=1, quantile_scores=False):
    ...
    if use_evidence is None:
        ...
        evidence_items = use_evidence
    if quantile_scores:
        df["p_init"] = df["p_init"].argsort().argsort()/len(df)
    for evidence in evidence_items:
        if labeled:
            for label in [0, 1]:
